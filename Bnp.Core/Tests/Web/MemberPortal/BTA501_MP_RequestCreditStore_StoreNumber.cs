...
{
    [TestClass]
    public class BTA501_MP_RequestCreditStore_StoreNumber : ProjectTestBase
    {
        ...
        public void BTA_501_MP_RequestCreditStore_StoreNumber()
        {
            ...
            try
            {
                ...
                testStep.SetOutput("LoyaltyNumber :" + basePages.GetLoyaltyNumber(member) + ",Name:" + member.FirstName + "Created Successfully");
                ...
                #region Step4:Search Transaction Store Number
                stepName = "Search With Transaction Store Number: " + RequestCredit_Search_Criteria.StoreNumber;
                testStep = TestStepHelper.StartTestStep(testStep);
                stepstatus = Mp_RequestCredit.Search_BasedOnStoreNumber(RequestCredit_Search_Criteria.TransactionNumber, RequestCredit_Search_Criteria.StoreNumber, MemberPortal_RequestCredit.TransactionType.Store.ToString(), out Step_Output);
                testStep.SetOutput(Step_Output);
                testStep = TestStepHelper.EndTestStep(testCase, testStep, stepName, stepstatus, DriverContext.SendScreenshotImageContent("WEB"));
                listOfTestSteps.Add(testStep);
                #endregion
