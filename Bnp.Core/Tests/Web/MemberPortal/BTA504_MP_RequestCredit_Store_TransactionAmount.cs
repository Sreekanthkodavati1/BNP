...
{
    [TestClass]
    public class BTA504_MP_RequestCredit_Store_TransactionAmount : ProjectTestBase
    {
        ...
        public void BTA_504_MP_RequestCredit_Store_TransactionAmount()
        {
            #region Objects
            ...
            try
            {
                ...
                stepName = "Search With Transaction Amount: " + RequestCredit_Search_Criteria.TxnAmount;
                testStep = TestStepHelper.StartTestStep(testStep);
                stepstatus = Mp_RequestCredit.Search_BasedOnTransactionAmount(RequestCredit_Search_Criteria.TransactionNumber, RequestCredit_Search_Criteria.TxnAmount, MemberPortal_RequestCredit.TransactionType.Store.ToString(), out Step_Output);
                testStep.SetOutput(Step_Output);
                testStep = TestStepHelper.EndTestStep(testCase, testStep, stepName, stepstatus, DriverContext.SendScreenshotImageContent("WEB"));
                listOfTestSteps.Add(testStep);
                #endregion
