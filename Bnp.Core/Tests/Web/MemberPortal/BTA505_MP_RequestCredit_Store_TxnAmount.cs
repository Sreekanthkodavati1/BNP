...
{
    [TestClass]
    public class BTA505_MP_RequestCredit_Store_TxnAmount : ProjectTestBase
    {
        ...
        public void BTA_505_MP_RequestCredit_Store_TxnAmount()
        {
            ...
            try
            {
                ...
                #region Step4:Search Transaction Register Number
                stepName = "Search With Transaction Register Number: " + RequestCredit_Search_Criteria.RegisterNumber;
                testStep = TestStepHelper.StartTestStep(testStep);
                stepstatus = Mp_RequestCredit.Search_BasedOnRegisterNumber(RequestCredit_Search_Criteria.TransactionNumber, RequestCredit_Search_Criteria.RegisterNumber, MemberPortal_RequestCredit.TransactionType.Store.ToString(), out Step_Output);
                testStep.SetOutput(Step_Output);
                testStep = TestStepHelper.EndTestStep(testCase, testStep, stepName, stepstatus, DriverContext.SendScreenshotImageContent("WEB"));
                listOfTestSteps.Add(testStep);
                #endregion
