...
{
    [TestClass]
    public class BTA507_MP_RequestCredit_Online_TxnWebOrderNumber : ProjectTestBase
    {
        ...
        public void BTA_507_MP_RequestCredit_Online_TxnWebOrderNumber()
        {
            ...
            try
            {
                ...
                #region Step4:Search With TxnWebOrderNumber
                stepName = "Select Online and Search with Txn Web OrderNumber: " + RequestCredit_Search_Criteria.OrderNumber;
                testStep = TestStepHelper.StartTestStep(testStep);
                stepstatus = Mp_RequestCredit.Search_BasedOnTxnWebOrderNmbr(RequestCredit_Search_Criteria.TransactionNumber, RequestCredit_Search_Criteria.OrderNumber, out Step_Output);
                testStep.SetOutput(Step_Output);
                testStep = TestStepHelper.EndTestStep(testCase, testStep, stepName, stepstatus, DriverContext.SendScreenshotImageContent("WEB"));
                listOfTestSteps.Add(testStep);
                #endregion
