namespace BnPBaseFramework.Web.Helpers
{
    ...
    public static class FilesHelper
    {
        ...
        public static string GetFolder(string downloadFolder, object currentDirectory)
        {
        ...
        /// <example>How to use it: <code>
        /// var filesNumber = FilesHelper.CountFiles(this.DriverContext.DownloadFolder, FileType.Txt);
        /// this.Driver.GetElement(this.fileLink.Format("some-file.txt")).ClickElement();
        ...
        /// this.Driver.GetElement(this.fileLink.Format(file), "Click on file").ClickElement();
        ...
        public static string ReadLogFile(string path)
        {
            ...
            {
                throw new Exception("Failed to read Log file from the given path " + path);
            }
        }

        /// <summary>
        /// To get word count form a text 
        /// </summary>
        /// <param name="word">The word to search</param>
        /// <param name="text">The text content</param>
        /// <returns>Count of a word</returns>
        public static int GetWordCount(string word, string text)
        {
            try
            {
                int count = 0;
                foreach (Match match in Regex.Matches(word, text))
                {
                    count++;
                }
                return count;
            ...
            {
                throw new Exception("Failed to get word count from the Text");
            }
        }
    }
}