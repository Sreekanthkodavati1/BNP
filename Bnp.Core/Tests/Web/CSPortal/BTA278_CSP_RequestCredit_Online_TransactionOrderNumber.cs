...
{
    [TestClass]
    public class BTA278_CSP_RequestCredit_Online_TransactionOrderNumber : ProjectTestBase
    {
        ...
        public void BTA_278_CSP_RequestCredit_Online_TransactionOrderNumber()
        {
            ...
            try
            {
                ...
                if (RequestCredit_Search_Criteria.OrderNumber.Equals(""))
                {
                    throw new Exception("Order Number Not Avaialble for Transaction:" + RequestCredit_Search_Criteria.TransactionNumber);
                }
                ...
                #region Step6:Search With Order Number
                stepName = "Select Online and Search With OrderNumber:" + RequestCredit_Search_Criteria.OrderNumber;
                testStep = TestStepHelper.StartTestStep(testStep);
                CSPortal_RequestCredit.Select_Online();
                stepstatus = CSPortal_RequestCredit.Search_Online_BasedOnOrderNumber(RequestCredit_Search_Criteria.TransactionNumber, RequestCredit_Search_Criteria.OrderNumber, out Step_Output);
                testStep.SetOutput(Step_Output);
                testStep = TestStepHelper.EndTestStep(testCase, testStep, stepName, stepstatus, DriverContext.SendScreenshotImageContent("WEB"));
                listOfTestSteps.Add(testStep);
                #endregion
